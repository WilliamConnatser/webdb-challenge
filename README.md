...
1. Explain the difference between `RDBMS` and `SQL`.
  ...
  RDBMS is the API that interacts with a database
  SQL is the actual database, and a language to interact directly with it.

1. Why do tables need a `primary key`?

Because each database item needs a unique key 
